...
static void delete_bulk_endpoints(struct adb_dev *dev)
{
	struct usb_request *req;
	int i;
...
void adb_function_del(struct usb_composite_dev *cdev,
	struct usb_configuration *c)
{
	struct adb_dev *dev = _adb_dev;

	if (dev) {
		usb_del_function(c, &dev->function);
		misc_deregister(&adb_device);
	}
